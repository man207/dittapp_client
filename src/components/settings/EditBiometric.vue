...
export default {
  ...
  methods: {
    onSubmit() {
      ...
      axios
        .post("/profile/biometric/edit", formData , config)
        .then((res) => {
          if (res) {
            this.snackBar = true
            this.$store.dispatch('setRecommended')
            setTimeout(() => {
  ...
  mounted() {
        if (!this.$store.state.recommended.calorie) {
          this.$router.push('/settings')
      }
                  let w = this.$store.state.recommended
            this.age = w.age;
